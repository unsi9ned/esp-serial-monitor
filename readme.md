...
- Возможность завершать передаваемые данные символом переноса строки (CR, LF, CR+LF), а также отправка спецсимволов CTRL+Z, ESP, что может быть полезно при работе с модемами
...
10. Нажать **Open BIN** и указать путь к прошивке, которая находится в каталоге **%TEMP%\arduino_build_xxxxx\yyyyy.ino.bin**, где *xxxxx* - случайны номер, который среда сгенерирует для текущей сборки, *yyyyy* - название скетча
